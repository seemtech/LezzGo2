...
-(void)MyProfileDetailsApi
{
    ...
    [manager POST:kServerurl parameters:params progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            NSLog(@"myprofile object %@",responseObject);
           // delegate.logininfo_Array=[responseObject valueForKey:@"show"];
            NSMutableArray  *array=[responseObject objectForKey:@"show"];
            NSMutableDictionary *dic=[array objectAtIndex:0];
            tf_username.text=[dic valueForKey:@"username"];
            tf_fullname.text=[dic objectForKey:@"full_name"];
            tf_age.text=[dic objectForKey:@"age"];
            tf_zipcode.text=[dic objectForKey:@"zip_code"];
            tf_password.text=[dic objectForKey:@"password"];
            tf_confirmpassword.text=[dic objectForKey:@"password"];
         
                SDWebImageManager *manager = [SDWebImageManager sharedManager];
                [manager downloadImageWithURL:[NSURL URLWithString: [dic valueForKey:@"profile_pic"] ]
                                      options:0
                                    ...
                                    completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                                        if (image) {
                                            [[SDImageCache sharedImageCache] storeImage:image forKey:[dic valueForKey:@"profile_pic"]];
                                            
                                            
                                            mainuserimage.layer.masksToBounds = YES;

                                            mainuserimage.layer.cornerRadius=mainuserimage.frame.size.height/2;
                                            mainuserimage.clipsToBounds=YES;
                                            mainuserimage.image = image;
...
//user_id , full_name , age , password , is_showpofilemap , is_notification
    ...
    params = @{@"action":@"profilesetting",@"user_id":delegate.userid,@"full_name":tf_fullname.text,@"age":tf_age.text,@"password":tf_password.text,@"zip_code":tf_zipcode.text};
                      ...
                      if (error) {
                        [SVProgressHUD dismiss];
                          ...
                          UIAlertAction* defaultAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action)
                                                          {
                                                              if([[responseObject objectForKey:@"msg"]isEqualToString:@"Profile updated"])
                                                              {
...
{
        if(tf_username.text.length!=0&&tf_fullname.text.length!=0&&tf_zipcode.text.length!=0&&tf_age.text.length!=0&&tf_password.text.length!=0&&tf_confirmpassword.text.length!=0&&[tf_password.text isEqualToString:tf_confirmpassword.text])
    {
        [self UpdatedetailsApi];
    }
    else
    {
