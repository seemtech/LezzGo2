...
+ (instancetype)sharedPushView
{
	@synchronized([self class])
	{
		if (!_sharedPushView){
            ...
            {
        nibArr = [[NSBundle mainBundle] loadNibNamed: @"AGPushNoteView_iPad" owner:self options:nil];

            }
            else
            {
               nibArr = [[NSBundle mainBundle] loadNibNamed: @"AGPushNoteView" owner:self options:nil];
...
- (void)setUpUI {
    CGRect f = self.frame;
    CGFloat width = [UIApplication sharedApplication].keyWindow.bounds.size.width;
    CGFloat height = isIOS7? 44: f.size.height;
